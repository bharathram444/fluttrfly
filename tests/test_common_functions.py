...
def test_with_loading_success():
    mock_task = MagicMock()
    with patch('time.sleep', return_value=None), patch('fluttrfly.bin.commands.global_variables.console.status') as mock_status:
...
def test_with_loading_file_not_found():
    ...
    with patch('time.sleep', return_value=None), patch('fluttrfly.bin.commands.global_variables.console.print') as mock_console_print:
        with_loading(mock_task, duration=1, status="Creating")
...
def test_write_yaml():
    ...
    with patch('builtins.open', mock_open()) as mock_file:
        with patch('yaml.dump') as mock_yaml_dump:
            ...
            mock_yaml_dump.assert_called_once_with(mock_data, mock_file(), default_flow_style=False, sort_keys=False)
